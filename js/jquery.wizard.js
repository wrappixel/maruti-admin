...
(function ($) {
  $.widget("ui.formwizard", {
    _init: function () {
      ...
      this.options.formOptions = $.extend(this.options.formOptions, {
        success: function (responseText, textStatus, xhr) {
          ...
          if (
            (wizard.options.formOptions &&
              wizard.options.formOptions.resetForm) ||
            !wizard.options.formOptions
          ) {
            wizard._reset();
          }
        ...
        beforeSubmit: function (arr, theForm, options) {
          if (formOptionsBeforeSubmit) {
            ...
            if (!shouldSubmit) wizard._enableNavigation();
            return shouldSubmit;
          }
        },
      ...
      if (this.options.historyEnabled) {
        $(window).bind("hashchange", undefined, function (event) {
          var hashStep =
            event.getState("_" + $(wizard.element).attr("id")) ||
            wizard.firstStep;
          if (hashStep !== wizard.currentStep) {
            if (
              wizard.options.validationEnabled &&
              hashStep === wizard._navigate(wizard.currentStep)
            ) {
            ...
            if (hashStep !== wizard.currentStep) wizard._show(hashStep);
          }
        });
      }
        ...
        if (options !== undefined) {
          ...
          options = $.extend({}, options, {
            success: function (data, statusText) {
              if (
                (success !== undefined && success(data, statusText)) ||
                success == undefined
              ) {
            ...
            complete: function (xhr, statusText) {
              if (complete !== undefined) complete(xhr, statusText);
    ...
    _animate: function (oldStep, newStep, stepShownCallback) {
      ...
      var wizard = this;
      old.animate(
        wizard.options.outAnimation,
        wizard.options.outDuration,
        wizard.options.easing,
        function () {
          current.animate(
            wizard.options.inAnimation,
            wizard.options.inDuration,
            wizard.options.easing,
            function () {
    ...
    _checkIflastStep: function (step) {
      this.isLastStep = false;
      if (
        $("#" + step).hasClass(this.options.submitStepClass) ||
    ...
    _state: function (state) {
      ...
      if (state !== undefined) return currentState[state];
